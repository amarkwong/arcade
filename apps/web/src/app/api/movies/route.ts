...
export async function GET(request: NextRequest) {
  ...
  if (!title || !title.trim()) {
    return NextResponse.json({ error: "Query parameter 'title' (or 'q') is required" }, { status: 400 });
  }

  if (!apiKey) {
    return NextResponse.json({ error: "OMDBAPI_KEY is not configured on the server" }, { status: 500 });
  }
  ...
  try {
    ...
    if (data?.Response === "False") {
      return NextResponse.json({ error: data?.Error || "Movie not found", raw: data }, { status: 404 });
    }
    ...
    return NextResponse.json({ error: "Failed to fetch movie data", detail: String(error) }, { status: 500 });
  }
}

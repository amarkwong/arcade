...
import MovieCard, { type Solver } from "./components/MovieCard/MovieCard";
...
const FilmojiGame: React.FC = () => {
	...
	useEffect(() => {
		...
		setPuzzles((prev) =>
			prev.map((puzzle) => {
				const solvedByUser = puzzle.solvedBy?.some(
					(solver) => Number(solver.userId) === Number(userId),
				);
				return { ...puzzle, unlocked: Boolean(solvedByUser) };
			}),
		);
	}, [isMounted, userId]);
		...
		setPuzzles((prev) =>
			prev.map((puzzle) => {
				...
				if (!isCorrect) {
					...
					setSnackbar(
						"Incorrect guesses shake the card, but you can try as many times as you like.",
					);
					snackbarTimer.current = setTimeout(() => setSnackbar(null), 3000);
					return { ...puzzle, guess };
				}
	...
	return (
		<div className="relative min-h-screen w-full bg-gradient-to-b from-slate-950 via-slate-900 to-black text-slate-50">
			...
			>
				...
				{activePuzzle && !isCreateOpen && !activePuzzle.unlocked ? (
					<motion.div
						key={activePuzzle.id}

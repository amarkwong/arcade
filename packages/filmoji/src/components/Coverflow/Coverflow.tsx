...
const Coverflow: React.FC<CoverflowProps> = ({ items, covers, onActiveIndexChange }) => {
  ...
  return (
    ...
    >
      ...
      {data
        .slice(Math.min(Math.max(0, activeIndex - 2), Math.max(data.length - 3, 0)), activeIndex + 3)
        .map((item, index) => {
          ...
          const rotation = originalIndex === activeIndex ? 0 : originalIndex > activeIndex ? -50 - 4 * distance : 50 + 4 * distance;
                  ...
                  <>
                    ...
                    <div className={styles.reflection}>
                      <Image
                        ...
                        className="absolute top-full h-full w-full -scale-y-100"
                      />
                    </div>
                  </>
                ) : (
